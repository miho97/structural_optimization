...
def get_args(normals, forces, density=0.4): 
  ...
  params = {
      ...
      'opt_steps': 80, 'print_every': 20}
  return ObjectView(params)



def mbb_beam_1(width=6, height=6, density=0.45, y=1, x=0):  
...
def mean_density(x, args, volume_contraint=False, use_filter=False):
  return anp.mean(physical_density(x, args, volume_contraint, use_filter)) / anp.mean(args.mask)
def objective_calc(x, args, volume_contraint=False, use_filter=False):
